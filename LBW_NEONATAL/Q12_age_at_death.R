...
AGE_MAX_HOURS <- 672  # 28 days — end of neonatal period; age values above this
                       # are treated as implausible data-entry errors and set to NA
...
analysis_df <- all_admissions %>%
    mutate(
        ...
        # Age at Admission (capped at AGE_MAX_HOURS = 672 h = 28 days)
        age_at_admission_hours_raw = suppressWarnings(as.numeric(age)),
        age_at_admission_hours     = if_else(
            is.finite(age_at_admission_hours_raw) & age_at_admission_hours_raw <= AGE_MAX_HOURS,
            age_at_admission_hours_raw, NA_real_
        ),
...
age_note <- sprintf("is.finite(age_at_death_days) & age_at_death_days >= 0 & age_at_admission <= %d h", AGE_MAX_HOURS)
