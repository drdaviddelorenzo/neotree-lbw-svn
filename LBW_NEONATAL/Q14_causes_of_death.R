...
# ===== RAW CAUSES FREQUENCY =====

cat("\n=== Calculating Raw Cause Frequencies ===\n")

# Base data with cleaned raw causes
raw_cause_base <- valid_nnd_df %>%
    mutate(
        raw_cause = str_trim(causedeath),
        raw_cause = ifelse(raw_cause == "" | is.na(raw_cause) | raw_cause == "NA", "Unknown", raw_cause)
    )

# By facility
raw_cause_freq <- raw_cause_base %>%
    group_by(facility) %>%
    mutate(n_total_deaths = n()) %>%
    group_by(facility, raw_cause, n_total_deaths) %>%
    summarise(n = n(), .groups = "drop") %>%
    mutate(pct = round(100 * n / n_total_deaths, 1)) %>%
    arrange(facility, desc(n)) %>%
    select(facility, causedeath = raw_cause, n, pct)

# Combined (both facilities)
raw_cause_combined <- raw_cause_base %>%
    mutate(n_total_deaths = n()) %>%
    group_by(raw_cause, n_total_deaths) %>%
    summarise(n = n(), .groups = "drop") %>%
    mutate(
        facility = "Combined",
        pct = round(100 * n / n_total_deaths, 1)
    ) %>%
    arrange(desc(n)) %>%
    select(facility, causedeath = raw_cause, n, pct)

raw_cause_freq_final <- bind_rows(raw_cause_freq, raw_cause_combined)

write_csv(raw_cause_freq_final, file.path(output_dir, "03_raw_cause_frequencies.txt"))
cat("Saved: 03_raw_cause_frequencies.txt\n")
