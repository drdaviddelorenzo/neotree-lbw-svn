...
#      Values > 672 h (28 days = end of neonatal period) are treated as implausible
#      data-entry errors and excluded from the summary statistics.
...
GA_MIN       <- 24
GA_MAX       <- 42
AGE_MAX_HOURS <- 672  # 28 days — end of neonatal period; values above this
                       # are treated as implausible data-entry errors
...
analysis_df <- all_admissions %>%
    mutate(
        ...
        # Values > AGE_MAX_HOURS (672 h = 28 days) are set to NA as implausible.
        age_hours_raw = suppressWarnings(as.numeric(age)),
        age_hours     = if_else(is.finite(age_hours_raw) & age_hours_raw <= AGE_MAX_HOURS,
                                age_hours_raw, NA_real_),
...
facility_stats <- analysis_df %>%
    ...
    summarise(
        ...
        # 3. Age on admission (capped at AGE_MAX_HOURS = 672 h)
        ...
        q1_age_hours       = round(quantile(age_hours, 0.25, na.rm = TRUE), 1),
        q3_age_hours       = round(quantile(age_hours, 0.75, na.rm = TRUE), 1),
        sd_age_hours       = round(sd(age_hours,   na.rm = TRUE), 1),
        n_age_missing      = sum(is.na(age_hours_raw)),
        n_age_capped       = sum(is.finite(age_hours_raw) & age_hours_raw > AGE_MAX_HOURS, na.rm = TRUE),
...
summary_table <- facility_stats %>%
    mutate(
        ...
        # Row 3: Mean (and median/quartiles) age on admission
        row_mean_age             = sprintf("%.1f hours (median: %.1f h; IQR: %.1f-%.1f)", 
                                           mean_age_hours, median_age_hours, q1_age_hours, q3_age_hours),
        row_age_note             = sprintf("n valid = %d; n missing/NA = %d; n excluded (>672 h) = %d",
                                           n_age_valid, n_age_missing, n_age_capped),
