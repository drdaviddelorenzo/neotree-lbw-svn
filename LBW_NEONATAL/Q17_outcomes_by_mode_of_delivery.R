...
# Analysis of Neonatal Outcomes by Mode of Delivery, LBW, and SGA Categories
#
# This script:
# 1. Cleans and processes Mode of Delivery (variable `modedelivery`).
# 2. Maps numeric codes to descriptive labels:
...
#    - Rows: Mode of Delivery (per Facility).
...
#
# Data sources:
# - KCH Malawi: Uses `modedelivery` (Numeric 1-7)
# - SMCH Zimbabwe: Uses `modedelivery` (Numeric 1-7)
...
map_mod <- function(x) {
    dplyr::case_when(
        x == "1" ~ "SVD",
        x == "2" ~ "Vacuum Extraction",
        x == "3" ~ "Forceps Extraction",
        x == "4" ~ "ELCS",
        x == "5" ~ "EMCS",
        x == "6" ~ "Breech Extraction",
        x == "7" ~ "Induced Vaginal",
        TRUE ~ NA_character_
    )
}
...
kch_proc <- kch_raw %>%
    mutate(
        facility = "KCH",
        mod_raw = if ("modedelivery" %in% names(.)) modedelivery else NA_character_
    )
...
analysis_df <- all_admissions %>%
    mutate(
        ...
        # Mode of Delivery Classification
        mod_desc = map_mod(as.character(mod_raw)),
...
# Filter for valid data (Valid Outcome) - Birthweight and Delivery managed appropriately
valid_df <- analysis_df %>%
    filter(outcome_norm %in% c("Alive", "Neonatal Death")) %>%
    mutate(
        # Handle Missing
        mod_desc = ifelse(is.na(mod_desc), "Missing", mod_desc),
        # Order interval factor
        mod_interval = factor(mod_desc, levels = MOD_LEVELS)
    )
...
calc_wide_outcome <- function(data, group_cols, group_levels) {
    ...
    summary_long <- data %>%
        ...
        group_by(facility, category = .data[[category_col]], mod_interval) %>%
        summarise(
    ...
    totals_long <- data %>%
        group_by(facility, mod_interval) %>%
        summarise(
            NND = sum(outcome_norm == "Neonatal Death"),
            DC = sum(outcome_norm == "Alive"),
            .groups = "drop"
    ...
    complete_grid <- expand_grid(
        ...
        mod_interval = MOD_LEVELS
    )

    summary_complete <- complete_grid %>%
        left_join(summary_long, by = c("facility", "category", "mod_interval")) %>%
    ...
    col_order <- c("facility", "mod_interval")
...
mod_note <- "map_mod(modedelivery): 1=SVD, 2=Vacuum, 3=Forceps, 4=ELCS, 5=EMCS, 6=Breech, 7=Induced; NA -> \"Missing\""
...
sample_size_summary <- tibble(
    description = c(
        ...
        "Analysis population — known mode of delivery",
        "Analysis population — missing mode of delivery",
    ...
    Filter_notes = c(
        ...
        paste0(outcome_note, " AND !is.na(map_mod(modedelivery))"),
        paste0(outcome_note, " AND is.na(map_mod(modedelivery)) — retained as \"Missing\" in tables"),
...
write_csv(lbw_wide, file.path(output_dir, "01_mod_outcomes_LBW_wide.csv"))
cat("Saved: 01_mod_outcomes_LBW_wide.csv\n")

write_csv(sga_wide, file.path(output_dir, "02_mod_outcomes_SGA_wide.csv"))
cat("Saved: 02_mod_outcomes_SGA_wide.csv\n")
