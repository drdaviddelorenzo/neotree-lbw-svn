...
run_analysis <- function(df, out_dir, label) {
    if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
    cat(sprintf("\n=== Running Analysis: %s ===\n", label))
    
    total_admissions <- nrow(df)
    cat("Total admissions in this sample:", total_admissions, "\n")
    
    if (total_admissions == 0) {
        cat("No records found for this sample. Skipping.\n")
        return(NULL)
    }

    valid_bw_count <- sum(df$is_valid_bw)
    ...
    sga_note <- paste0(bw_note, " AND ", ga_note, " AND !is.na(sga_status)")
    ...
    sample_size_summary <- tibble(
        ...
        n = c(
            ...
            sum(!df$is_valid_bw),
            sum(df$is_valid_sga),
            sum(df$sga_explanation == "Missing BW"),
            sum(df$sga_explanation == "Missing GA"),
            sum(df$sga_explanation == "Missing both BW and GA")
        ),
    ...
    # ----- SAVE OUTPUTS -----
    write_csv(sample_size_summary, file.path(out_dir, "01_sample_size_summary.csv"))
    write_csv(all_overall_stats, file.path(out_dir, "02_admissions_by_category_overall.csv"))
    write_csv(all_facility_stats, file.path(out_dir, "03_admissions_by_category_and_facility.csv"))
...
run_analysis(analysis_df, "Q8_outputs", "Full Sample")

# 2. USS Subset Analysis (Filtered from Full Sample)
if ("methodestgest" %in% names(analysis_df)) {
    uss_df <- analysis_df %>% filter(toupper(trimws(methodestgest)) == "USS")
    run_analysis(uss_df, "Q8_outputs_USS", "USS Subset (filtered from full data)")
} else {
    cat("\nVariable 'methodestgest' not found. Skipping USS subset analysis.\n")
}
