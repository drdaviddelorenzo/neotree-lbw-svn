...
gender_note <- "map_gender(gender): M=Male, F=Female, NS/U=Unsure; NA/blank -> \"Missing\""
...
sample_size_summary <- tibble(
    description = c(
        ...
        "Analysis population — Male",
        "Analysis population — Female",
        "Analysis population — Unsure",
        "Analysis population — missing gender",
    ...
    n = c(
        ...
        sum(valid_df$gender_desc == "Male"),
        sum(valid_df$gender_desc == "Female"),
        sum(valid_df$gender_desc == "Unsure"),
        sum(valid_df$gender_desc == "Missing"),
    ...
    Filter_notes = c(
        ...
        paste0(outcome_note, " AND toupper(trimws(gender)) == \"M\""),
        paste0(outcome_note, " AND toupper(trimws(gender)) == \"F\""),
        paste0(outcome_note, " AND toupper(trimws(gender)) %in% c(\"NS\", \"U\")"),
        paste0(outcome_note, " AND gender is NA/blank/unrecognised — retained as \"Missing\" in tables"),
