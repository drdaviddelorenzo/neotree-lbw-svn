...
2. **Calculates Age at Death**:
    ...
    * **Age cap**: values of `age > 672 hours` (> 28 days, i.e. beyond the neonatal period) are treated as implausible data-entry errors and set to `NA` before computing Age at Death.
...
* **Variables Used**:
  ...
  * `age`: Age at admission (in hours). Values > 672 h (28 days) are excluded as implausible.
...
| Variable | Rule | Threshold / Constant |
|---|---|---|
| Birth weight | `is.finite(bw_kg) & bw_kg >= BW_MIN_KG & bw_kg <= BW_MAX_KG` | 0.3 – 7.0 kg |
| Gestational age | `gestation >= GA_MIN & gestation <= GA_MAX` | 24 – 42 weeks |
| Age at admission | `age_hours_raw <= AGE_MAX_HOURS` (implausible values → NA) | ≤ 672 h (28 days) |
