...
# Sample size summary
# Note: HIV groups are NOT mutually exclusive — "HIV True (Tested)" is a superset
# of "HIV True & Reactive", "HIV True & Non-Reactive", and "HIV True & Unknown".
# Counts therefore do not sum to the analysis population total.
bw_note <- sprintf(
...
sample_size_summary <- tibble(
    description = c(
        ...
        "HIV group: HIV False (No Test)  [norm_bool(mathivtest) == FALSE]",
        "HIV group: HIV True (Tested)  [norm_bool(mathivtest) == TRUE]",
        "HIV group: HIV True & Reactive  [tested == TRUE & hivtestresult == \"R\"]",
        "HIV group: HIV True & Non-Reactive  [tested == TRUE & hivtestresult == \"NR\"]",
        "HIV group: HIV True & Unknown result  [tested == TRUE & result_norm is Unknown/NA]",
        "HIV group: HIV test NA  [is.na(norm_bool(mathivtest))]",
        "NOTE: HIV True sub-groups (Reactive/Non-Reactive/Unknown) sum to HIV True (Tested) — groups are NOT mutually exclusive",
    ...
    n = c(
        ...
        sum(valid_df$tested == FALSE, na.rm = TRUE),
        sum(valid_df$tested == TRUE, na.rm = TRUE),
        sum(valid_df$tested == TRUE & valid_df$result_norm == "Reactive", na.rm = TRUE),
        sum(valid_df$tested == TRUE & valid_df$result_norm == "Non-Reactive", na.rm = TRUE),
        sum(valid_df$tested == TRUE & (valid_df$result_norm == "Unknown" | is.na(valid_df$result_norm)), na.rm = TRUE),
        sum(is.na(valid_df$tested)),
        NA_integer_,
    ...
    Filter_notes = c(
        ...
        paste0(outcome_note, " AND norm_bool(mathivtest) == FALSE"),
        paste0(outcome_note, " AND norm_bool(mathivtest) == TRUE"),
        paste0(outcome_note, " AND tested == TRUE & result_norm == \"Reactive\""),
        paste0(outcome_note, " AND tested == TRUE & result_norm == \"Non-Reactive\""),
        paste0(outcome_note, " AND tested == TRUE & (result_norm == \"Unknown\" | is.na(result_norm))"),
        paste0(outcome_note, " AND is.na(norm_bool(mathivtest))"),
        NA_character_,
