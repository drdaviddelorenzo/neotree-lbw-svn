...
# Q2_birth_insights_maternal_data.R, Q3_live_births_by_category.R, and
# Q4_stillbirths_by_category.R so that sample sizes are comparable.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# COLUMN-SPECIFIC BIRTH WEIGHT PARSER — SMCH ONLY
# ---------------------------------------------------------------------------
# Raw format audit (confirmed from data):
#
#   SMCH (ZIM) bwtdis — plain numeric, kg  e.g. "3.0"  (all values <= 10)
#
# Q5 uses SMCH data only; bwtdis is the sole birth weight source.
# Parsing is explicit — no median-based unit inference.
# Returns values in KILOGRAMS for use in bw_category_from_kg() / classify_sga().

# SMCH bwtdis: plain numeric, already in kg -> use directly
...
# Parse birth weight — bwtdis is the sole source for SMCH, plain numeric in kg
cat("Parsing birthweight (SMCH bwtdis, kg)...\n")
smch_raw <- smch_raw %>%
    mutate(
...
process_ennd <- ennd_all %>%
    mutate(
        ...
        # Validations — thresholds from shared constants (match Q2–Q4 and maternal_descriptive_analysis.R)
...
sample_size_summary <- tibble(
    ...
    Filter_notes = case_when(
        grepl("^Total SMCH", description, ignore.case = TRUE) ~ "All SMCH records",
        grepl("Early neonatal deaths|^ENND$", description, ignore.case = TRUE) ~ "neotreeoutcome == \"ENND\"",
...
smch_all_classified <- smch_data %>%
    mutate(
        # Validity thresholds from shared constants — must match process_ennd above
        is_valid_bw = is.finite(bw_kg) & bw_kg >= BW_MIN_KG & bw_kg <= BW_MAX_KG,
