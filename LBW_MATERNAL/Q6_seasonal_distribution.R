...
# Must match the constants in maternal_descriptive_analysis.R and Q2–Q5
...
kch_raw <- kch_raw %>% mutate(bw_kg = build_kch_bw_kg(.), facility = "KCH")
...
smch_raw <- smch_raw %>% mutate(bw_kg = parse_smch_bwtdis_kg(bwtdis), facility = "SMCH")
...
all_maternal_enhanced <- all_maternal %>%
    mutate(
        ...
        # Validations — thresholds from shared constants (match Q2–Q5 and maternal_descriptive_analysis.R)
...
sample_size_summary <- tibble(
    ...
    Filter_notes = case_when(
        ...
        grepl("valid season", description, ignore.case = TRUE) ~
            "!is.na(dateadmission) & dateadmission parses to a valid date",
        grepl("missing date", description, ignore.case = TRUE) ~
            "dateadmission missing or unparseable",
        grepl("valid birthweight|LBW sample", description, ignore.case = TRUE) ~
            sprintf("is.finite(bw_kg) & bw_kg >= %.1f kg & bw_kg <= %.1f kg", BW_MIN_KG, BW_MAX_KG),
        grepl("Discarded.*LBW|missing.*invalid birthweight", description, ignore.case = TRUE) ~
            sprintf("NOT (is.finite(bw_kg) & bw_kg >= %.1f kg & bw_kg <= %.1f kg)", BW_MIN_KG, BW_MAX_KG),
        grepl("valid SGA", description, ignore.case = TRUE) ~
            sprintf("BW valid AND !is.na(gestation) & gestation >= %d wks & gestation <= %d wks", GA_MIN, GA_MAX),
