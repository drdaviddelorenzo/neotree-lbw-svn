...
MAX_LOS_DAYS <- Inf # Filter out stays longer than 90 days
...
analysis_df <- all_admissions %>%
    mutate(
        ...
        los_days = suppressWarnings(as.numeric(lengthofstay)),
...
write_csv(sample_summary, file.path(output_dir, "00_sample_size_summary.csv"))
