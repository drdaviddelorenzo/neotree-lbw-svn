# ==============================================================================
# fig2A_stillbirth_risk_zim.R
# FIGURE 2A — Stillbirth Odds Ratio by SVN Category: ALL Stillbirths
...
cat("\n===== Figure 2A: Stillbirth odds ratio — All stillbirths =====\n")
...
or_smch <- or_raw %>%
  filter(
    ...
    sb_scope == "All stillbirths"
  ) %>%
  filter(category %in% SVN_CATS)

ref_row <- tibble(
  scope = "SMCH", sb_scope = "All stillbirths", category = "Term-AGA",
...
fig2A <- ggplot(or_plot, aes(x = svn_cat)) +
  geom_errorbar(
  ...
  labs(
    title    = "Stillbirth odds ratio by SVN category - All stillbirths, Zimbabwe (SMCH), 2022-2025",
...
save_figure(fig2A, "fig2A_stillbirth_risk_zim", width = 8, height = 7)
cat("Figure 2A complete.\n")
