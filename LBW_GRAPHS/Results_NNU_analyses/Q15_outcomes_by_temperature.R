...
valid_note <- paste0(outcome_note, " AND ", temp_note)
...
sample_size_summary <- tibble(
    description = c(
        "Total admissions",
        "Analysis population (valid outcome AND valid temperature)",
        "Excluded: missing/invalid outcome",
        "Excluded: missing/invalid temperature",
    ...
    n = c(
        ...
        sum(is.na(analysis_df$temp_val) | analysis_df$temp_val < MIN_TEMP | analysis_df$temp_val > MAX_TEMP),
