...
age_note <- sprintf(
    "is.finite(mat_age_val) & mat_age_val >= %d yrs & mat_age_val <= %d yrs",
    MIN_AGE, MAX_AGE
)
outcome_note <- "normalise_outcome(neotreeoutcome) %in% c(\"Alive\", \"Neonatal Death\")"
valid_note <- paste0(outcome_note, " AND ", age_note)
...
sample_size_summary <- tibble(
    description = c(
        "Total admissions",
        "Analysis population (valid outcome AND valid maternal age)",
        "Excluded: missing/invalid outcome",
        "Excluded: missing/invalid maternal age",
    ...
    n = c(
        ...
        sum(is.na(analysis_df$mat_age_val) | analysis_df$mat_age_val < MIN_AGE | analysis_df$mat_age_val > MAX_AGE),
    ...
    Filter_notes = c(
        ...
        paste0("NOT (", age_note, ")"),
        paste0(valid_note, " AND ", bw_note),
        paste0(valid_note, " AND NOT (", bw_note, ")"),
        paste0(valid_note, " AND ", sga_note),
